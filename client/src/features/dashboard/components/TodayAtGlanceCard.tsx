...
import { LogWorkoutDialog } from '@/features/optimize/components/workout/LogWorkoutDialog';
          ...
          <div className="p-3 md:p-4 rounded-xl bg-white border border-[#1B4332]/10">
            ...
            <div className={isMobile ? "space-y-3" : "flex items-center justify-between"}>
              <Link href="/dashboard/optimize/workout" className={isMobile ? "flex items-center gap-3" : "flex items-center gap-3 flex-1"}>
                <div className={`p-2.5 md:p-2 rounded-lg flex-shrink-0 ${data.workoutCompleted || data.isRestDay
                  ? 'bg-green-100'
                  : data.hasWorkoutToday
                    ? 'bg-[#D4A574]/20'
                    : 'bg-gray-100'
                  }`}>
                  <Dumbbell className={`h-5 w-5 ${data.workoutCompleted || data.isRestDay
                    ? 'text-green-600'
                    : data.hasWorkoutToday
                      ? 'text-[#D4A574]'
                      : 'text-gray-400'
              ...
              <div className={isMobile ? "flex gap-2 w-full" : "flex items-center gap-2 flex-shrink-0"}>
                {data.workoutCompleted ? (
                  ...
                  <Button
                    size="sm"
                    className={`text-xs bg-green-600 hover:bg-green-700 text-white touch-feedback ${isMobile ? 'flex-1 h-10' : ''
                      }`}
                    onClick={(e) => {
                      e.preventDefault();
