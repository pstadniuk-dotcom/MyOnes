      ...
      <CardContent>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <p className="text-2xl font-bold">{data.totalFormulas}</p>
            <p className="text-xs text-muted-foreground">Total Formulas</p>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <p className="text-2xl font-bold">{data.averageIngredients}</p>
            <p className="text-xs text-muted-foreground">Avg Ingredients</p>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <p className="text-2xl font-bold">{(data.averageTotalMg / 1000).toFixed(1)}g</p>
            <p className="text-xs text-muted-foreground">Avg Formula Size</p>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <p className="text-2xl font-bold">{data.customizationRate}%</p>
                ...
                <div key={base.name} className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm truncate max-w-[150px]">{base.name}</span>
            ...
            <div className="space-y-2">
              {data.popularAdditions.slice(0, 5).map((addition, index) => (
                <div key={addition.name} className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm truncate max-w-[150px]">{addition.name}</span>
                      <Badge variant="outline" className="text-xs">
                        {addition.percentage}%
                      </Badge>
                    </div>
