...
export default function CheckoutPage() {
  ...
  const [includeMembership, setIncludeMembership] = useState(membershipParam === '1');
  ...
  const formulaPrice = quoteData?.quote?.available ? (quoteData.quote.total ?? 0) : 0;
  const membershipMonthlyPrice = membershipTier ? membershipTier.priceCents / 100 : 0;
  ...
  const showDiscounted = hasActiveMembership || (includeMembership && membershipUpsellAvailable);
...
// }, []);
  ...
  const checkoutMutation = useMutation({
    mutationFn: async (paymentToken: string) => {
      const response = await apiRequest('POST', '/api/billing/checkout', {
        paymentToken,
        formulaId,
        includeMembership: membershipUpsellAvailable ? includeMembership : false,
        enableAutoShip,
        plan: 'monthly',
        shippingAddress: {
          firstName,
  ...
  const handlePlaceOrder = useCallback(
    (e: React.FormEvent) => {
      ...
      if (!firstName || !lastName || !line1 || !city || !state || !zip) {
        toast({ title: 'Missing information', description: 'Please fill in all required shipping fields.', variant: 'destructive' });
        return;
      }
      if (!email) {
      ...
      if (!billingSameAsShipping && (!billingFirstName || !billingLastName || !billingLine1 || !billingCity || !billingState || !billingZip)) {
        toast({ title: 'Missing information', description: 'Please fill in all required billing address fields.', variant: 'destructive' });
        return;
      }
      setValidationError(null);
      setSubmitting(true);
  ...
  if (!formulaId) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#ede8e2' }}>
        <div className="text-center space-y-4">
          <p className="text-lg text-[#262626]/70">No formula selected.</p>
          <Button onClick={() => navigate('/dashboard/formula')} style={{ backgroundColor: '#054700' }} className="text-white">
            Go to My Formula
          </Button>
        </div>
      </div>
              ...
              <form onSubmit={handlePlaceOrder} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <FormField label="First Name" required>
                        <Input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="First name" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                      </FormField>
                      <FormField label="Last Name" required>
                        <Input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Last name" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                      </FormField>
                    </div>
                    <FormField label="Address" required>
                      <Input value={line1} onChange={e => setLine1(e.target.value)} placeholder="Street address" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                    </FormField>
                    <FormField label="Apartment, suite, etc.">
                      <Input value={line2} onChange={e => setLine2(e.target.value)} placeholder="Apt, suite, unit (optional)" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" />
                    </FormField>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      <FormField label="City" required className="col-span-2 sm:col-span-1">
                        <Input value={city} onChange={e => setCity(e.target.value)} placeholder="City" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                      </FormField>
                      ...
                      <FormField label="ZIP Code" required>
                        <Input value={zip} onChange={e => setZip(e.target.value)} placeholder="12345" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" maxLength={10} required />
                      </FormField>
                    </div>
                  </div>
                </FormSection>
                ...
                <FormSection title="Billing Address">
                  <div className="space-y-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-[#262626]/70">Same as shipping address</span>
                    </label>
                    ...
                    {!billingSameAsShipping && (
                      <div className="space-y-4 pt-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <FormField label="First Name" required>
                            <Input value={billingFirstName} onChange={e => setBillingFirstName(e.target.value)} placeholder="First name" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                          </FormField>
                          <FormField label="Last Name" required>
                            <Input value={billingLastName} onChange={e => setBillingLastName(e.target.value)} placeholder="Last name" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                          </FormField>
                        </div>
                        <FormField label="Address" required>
                          <Input value={billingLine1} onChange={e => setBillingLine1(e.target.value)} placeholder="Street address" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                        </FormField>
                        <FormField label="Apartment, suite, etc.">
                          <Input value={billingLine2} onChange={e => setBillingLine2(e.target.value)} placeholder="Apt, suite, unit (optional)" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" />
                        </FormField>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                          <FormField label="City" required className="col-span-2 sm:col-span-1">
                            <Input value={billingCity} onChange={e => setBillingCity(e.target.value)} placeholder="City" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" required />
                          </FormField>
                          <FormField label="State" required>
                            <select
                              value={billingState}
                          ...
                          <FormField label="ZIP Code" required>
                            <Input value={billingZip} onChange={e => setBillingZip(e.target.value)} placeholder="12345" className="bg-white border-black/10 focus:border-[#054700] focus:ring-[#054700]/20" maxLength={10} required />
                          </FormField>
                        </div>
                      </div>
                    )}
                  ...
                  {scriptError ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-[#262626]/40">Accepted:</span>
                ...
                <div className="rounded-2xl bg-white/60 border border-black/[0.04] p-5 space-y-4">
                  <div className="flex flex-wrap items-center justify-center gap-6">
                    <SecurityBadge icon={<Lock className="w-4 h-4" />} text="PCI DSS Compliant" />
                    <SecurityBadge icon={<Shield className="w-4 h-4" />} text="256-bit SSL" />
                    <SecurityBadge icon={<ShieldCheck className="w-4 h-4" />} text="HIPAA Secure" />
                  </div>
                  ...
                  <p className="text-[10px] text-center text-[#262626]/30 leading-relaxed">
                    ...
                    <br />30-day money-back guarantee on all orders.
                  </p>
                </div>
              </form>
            </section>
  ...
  return (
    <div className="rounded-2xl bg-white p-6 sm:p-8 shadow-sm border border-black/[0.04] space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-[#262626] tracking-tight">What Our Customers Say</h3>
      ...
      <div className="space-y-4">
        {checkoutReviews.map((review) => (
          <div key={review.name} className="border-t border-black/[0.04] pt-4 first:border-0 first:pt-0">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: '#054700' }}>
                {review.name.charAt(0)}
              </div>
              <div>
                <p className="text-sm font-medium text-[#262626] leading-tight">{review.name}</p>
            ...
            <p className="text-xs text-[#262626]/60 leading-relaxed">"{review.text}"</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-center gap-4 pt-2 border-t border-black/[0.04]">
        <div className="text-center">
          ...
          <p className="text-[10px] text-[#262626]/40 uppercase tracking-wider">Clinical Patients</p>
        </div>
        <div className="w-px h-8 bg-black/[0.06]" />
        <div className="text-center">
          <p className="text-lg font-bold text-[#054700]">94%</p>
          <p className="text-[10px] text-[#262626]/40 uppercase tracking-wider">Report Improvement</p>
        </div>
        <div className="w-px h-8 bg-black/[0.06]" />
        <div className="text-center">
          <p className="text-lg font-bold text-[#054700]">8+ Yrs</p>
          <p className="text-[10px] text-[#262626]/40 uppercase tracking-wider">Experience</p>
        </div>
      </div>
    </div>
  );
...
function FormSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-white p-6 sm:p-8 shadow-sm border border-black/[0.04]">
      <h2 className="text-lg font-semibold text-[#262626] mb-5 tracking-tight">{title}</h2>
      {children}
    </div>
  );
}
...
function CardBrandIcon({ brand }: { brand: 'visa' | 'mastercard' | 'amex' | 'discover' }) {
  const images: Record<string, { src: string; alt: string }> = {
    visa: { src: '/credit%20cards/1490135017-visa_82256.png', alt: 'Visa' },
    mastercard: { src: '/credit%20cards/1490135018-mastercard_82253.png', alt: 'Mastercard' },
    amex: { src: '/credit%20cards/1490135020-american-express_82257.png', alt: 'American Express' },
    discover: { src: '/credit%20cards/1490135012-discover_82255.png', alt: 'Discover' },
  };
...
function OrderSummaryCard({
  ...
  return (
    <div className="rounded-2xl bg-white p-6 sm:p-8 shadow-sm border border-black/[0.04] space-y-6">
      <h2 className="text-lg font-semibold text-[#262626] tracking-tight">Order Summary</h2>
          ...
          {quote && (
            <div className="mt-1 space-y-0.5">
              <p className="text-sm text-[#262626]/50">
                {quote.capsuleCount} capsules/day · {quote.totalCapsules} total · {quote.weeks}-week supply
              </p>
              <p className="text-sm text-[#262626]/50">
                {quote.mappedIngredients} active ingredient{quote.mappedIngredients !== 1 ? 's' : ''}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="space-y-3 text-sm">
        <div className="flex justify-between">
          <span className="text-[#262626]/60">Formula ({quote?.weeks ?? 4}-week supply)</span>
          {showDiscounted ? (
            <span className="flex items-center gap-2">
              <span className="line-through text-[#262626]/30">${formulaPrice.toFixed(2)}</span>
              <span className="font-medium text-[#054700]">${discountedPrice.toFixed(2)}</span>
            </span>
          ) : (
            <span className="font-medium text-[#262626]">${formulaPrice.toFixed(2)}</span>
          )}
        </div>
        ...
        {includeMembership && membershipUpsellAvailable && (
          <div className="flex justify-between">
            <span className="text-[#262626]/60">{tierName || 'Membership'} (monthly)</span>
            <span className="font-medium text-[#262626]">${membershipMonthlyPrice.toFixed(2)}</span>
          </div>
        )}

        <div className="flex justify-between">
          <span className="text-[#262626]/60">Shipping</span>
          <span className="font-medium text-[#262626]">{shippingCost > 0 ? `$${shippingCost.toFixed(2)}` : 'Free'}</span>
        </div>
      </div>
      ...
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-2xl font-bold text-[#262626] tracking-tight">${orderTotal.toFixed(2)}</span>
      </div>
      ...
      {membershipUpsellAvailable && (
        <>
          <Separator className="bg-black/[0.06]" />
          <div className="rounded-xl border-2 p-4 transition-colors" style={{ borderColor: includeMembership ? '#054700' : 'rgba(0,0,0,0.06)', backgroundColor: includeMembership ? '#054700/[0.03]' : 'transparent' }}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Sparkles className="w-4 h-4 text-[#D4A574]" />
                  <span className="font-semibold text-sm text-[#262626]">Add {tierName || 'Membership'}</span>
                </div>
                <p className="text-xs text-[#262626]/50 leading-relaxed">
                  Save ${membershipSavings.toFixed(2)} on this order and get 15% off all future orders, priority support, and free shipping.
                </p>
              </div>
              <Switch
      ...
      {hasActiveMembership && (
        <>
          <Separator className="bg-black/[0.06]" />
          <div className="flex items-center gap-2 rounded-lg p-3" style={{ backgroundColor: 'rgba(5,71,0,0.05)' }}>
            <Check className="w-4 h-4 text-[#054700]" />
            <span className="text-sm font-medium text-[#054700]">Member discount applied</span>
          </div>
        </>
      )}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm text-[#262626]">Auto-Ship</span>
            </div>
            <p className="text-xs text-[#262626]/50 leading-relaxed">
              Automatically reorder every {quote?.weeks ?? 4} weeks. Cancel or pause anytime.
            </p>
          </div>
          <Switch
