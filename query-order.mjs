...
const c = new pg.Client({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
await c.connect();
const sql = `SELECT id, user_id, formula_id, status, amount_cents, manufacturer_cost_cents, supply_weeks, placed_at, shipped_at
             FROM orders
             WHERE UPPER(id) LIKE '%EBB7145E%'
             ORDER BY placed_at DESC
             LIMIT 5`;
...
const c = new pg.Client({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
await c.connect();
const r = await c.query(SELECT id, user_id, formula_id, status, amount_cents, manufacturer_cost_cents, supply_weeks, placed_at, shipped_at FROM orders WHERE UPPER(id) LIKE 'EBB7145E%' OR UPPER(id) LIKE '%EBB7145E%' LIMIT 5);
console.log('rows:', r.rows.length);
for (const row of r.rows) console.log(JSON.stringify(row, null, 2));
await c.end();
