...
// Re-export the AI image generator so existing callers can import from here
export { generateBlogImage } from './blogImageService';
...
IMPORTANT: Return only the JSON object, no preamble, no markdown fences.`;
  ...
  // Image will be generated after article creation (async, separate step)
  // so we leave it null here — callers handle image generation
  const featuredImage: string | null = null;
