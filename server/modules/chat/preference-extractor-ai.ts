/**
 * AI-based preference extractor — semantic fallback for the regex
 * detector in `preference-detector.ts`.
 *
 * WHY THIS EXISTS:
 *   The regex detector is fast and free, but brittle by design — it
 ...
 *
 * NEVER throws — on any failure (timeout, parse error, validation
...
/**
 * Heuristic gate — when should we burn an AI call on intent extraction?
 *
 * Matches when a message contains a removal *signal* even if the regex
 * detector didn't match. Deliberately broad — false positives just cost
...
/**
 * Inverse signal — the message is clearly NOT a removal intent.
...
export function shouldRunAIExtractor(message: string, regexFoundCount: number): boolean {
  ...
  // is to catch what regex missed — not to second-guess.
  ...
  // and skip — we don't want to accidentally blacklist something the user
  // is asking to add.
...
You will be given:
  ...
  • Optionally, a brief snippet of recent conversation context
  • A list of approved supplement ingredient names
...
CRITICAL RULES:
  ...
  2. Use EXACT names from the approved list — case-sensitive, including suffixes like "Cinnamon 20:1" or "Ginkgo Biloba Extract 24%". Never invent names.
  3. If the user is ASKING for an ingredient ("add X", "include X", "more X") — DO NOT include it.
  4. If the user is asking a question about an ingredient ("what does X do") — DO NOT include it.
  5. If the user is referencing an ingredient neutrally ("my doctor mentioned X") — DO NOT include it.
  6. If the message contains no clear removal intent, return {"rejected": []}.
  7. Common short forms / typos map to canonical names (e.g. "ginko" → "Ginkgo Biloba Extract 24%", "cinnamon" → "Cinnamon 20:1", "hawthorn" → "Hawthorn Berry", "coq10" → "CoEnzyme Q10"). Resolve them to the catalog name.
...
Examples:
  ...
  → {"rejected": ["Ginger Root", "Resveratrol"]}
...
export async function extractRejectionsWithAI(
  ...
  try {
    ...
    logger.warn('AI preference extractor call failed — falling back to regex result', {
      error: err?.message,
    });
