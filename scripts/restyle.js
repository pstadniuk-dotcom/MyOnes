...
// Now fix text-white → text-gray-900 EXCEPT on primary buttons (bg-[#054700])
// Strategy: replace all text-white with text-gray-900, then fix buttons back
content = content.split('text-white').join('text-gray-900');

// Fix buttons: any className containing bg-[#054700] should have text-white not text-gray-900
content = content.replace(/className="([^"]*bg-\[#054700\][^"]*)"/g, (match, classes) => {
  return 'className="' + classes.replace(/text-gray-900/g, 'text-white') + '"';
});

// Fix buttons that use bg-amber-500 (yellow primary buttons)
content = content.replace(/className="([^"]*bg-amber-500[^"]*)"/g, (match, classes) => {
  return 'className="' + classes.replace(/text-gray-900/g, 'text-white') + '"';
});

// Fix buttons that use bg-blue-600 (blue action buttons)
content = content.replace(/className="([^"]*bg-blue-600[^"]*)"/g, (match, classes) => {
  return 'className="' + classes.replace(/text-gray-900/g, 'text-white') + '"';
});

// Fix template literals with bg-[#054700] (ternary classNames)
content = content.replace(/`([^`]*bg-\[#054700\][^`]*)`/g, (match, inner) => {
  return '`' + inner.replace(/text-gray-900/g, 'text-white') + '`';
});

// Fix the ternary patterns like: favOnly ? 'bg-amber-500 hover:bg-amber-400 text-gray-900' 
content = content.replace(/'([^']*bg-amber-500[^']*)'/g, (match, inner) => {
  return "'" + inner.replace(/text-gray-900/g, 'text-white') + "'";
});
content = content.replace(/'([^']*bg-\[#054700\][^']*)'/g, (match, inner) => {
  return "'" + inner.replace(/text-gray-900/g, 'text-white') + "'";
